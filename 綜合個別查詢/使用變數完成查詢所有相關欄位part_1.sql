...
;WITH 
	...
	A AS (-- 此為上班日(包含休假日)
	   	...
	   	FROM (
	   			SELECT -- 0為上班日，1為六日
		            ...
		            WHEN '0' 
		            THEN CONVERT(varchar, SWITCHOFFSET(工時紀錄表.IN_START_TIME, '+08:00'), 111) 
		            END AS 上班日,
					角色.NAME AS 報工者,
        ...
        FROM
            (
            	SELECT 
	            	CASE 行事曆單身.day_off 
	            	WHEN '1' 
	            	THEN CONVERT(varchar, SWITCHOFFSET(行事曆單身.DAY_DATE, '+08:00'), 111) 
	            	END AS 休假日
		        FROM [innovator].[IN_TIMERECORD] AS 工時紀錄表
		        LEFT JOIN [innovator].[BUSINESS_CALENDAR_EXCEPTION] AS 行事曆單身 
		         	ON 	CONVERT(varchar, SWITCHOFFSET(工時紀錄表.IN_START_TIME, '+08:00'), 111) 
		         		= 
		         		CONVERT(varchar, SWITCHOFFSET(行事曆單身.DAY_DATE, '+08:00'), 111))AS cc
    ...
    B AS ( -- 區間內總工時(他在Aras有報的工時)

		SELECT 工時紀錄表.報工者,
		       SUM(工時紀錄表.工時) AS 實際總工時
		FROM A3 AS 工時紀錄表
		WHERE 工時紀錄表.上班日 BETWEEN @startDate AND @endDate
		GROUP BY 工時紀錄表.報工者 
		), 
	B1 AS ( -- -- 實際總工時(他在Aras有報的工時
    ...
    C AS (-- 計算不重複的日期
         ...
         SELECT COUNT(DISTINCT A3.上班日) AS 上班天數,
                A3.報工者
         FROM [innovator].[IN_TIMERECORD] AS 工時紀錄表
