		  ...
		  AND NOT EXISTS
			(SELECT *
			 ...
			 WHERE cc.休假日 IS NOT NULL
			   AND bb.非假日 = cc.休假日)
			AND bb.報工者='0271 楊承檍'
				AND (bb.非假日 BETWEEN CONVERT(VARCHAR(10),DATEADD(MONTH,-5,getdate()),111) AND CONVERT(VARCHAR(10),DATEADD(MONTH,-3,getdate()),111))
			GROUP BY bb.報工者
--
) AS cc
WHERE 角色.NAME ='0271 楊承檍'
